...
def _tor_check(proxy):
    if proxy:
        return
    try:
        s = socket.create_connection(("127.0.0.1", 9050), timeout=1)
        s.close()
        print(f"\n{Y}[?] TOR detected on port 9050 but --proxy is not set.{W}")
        print(f"    {C}Tip: add '--proxy socks5://127.0.0.1:9050' to route traffic anonymously.{W}\n")
    except Exception:
        pass

def run_recon(domain, args):
    _tor_check(getattr(args, 'proxy', None))
