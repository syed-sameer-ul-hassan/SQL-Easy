...
In v1.1.0 it also runs **Nuclei** after SQLMap to catch other vulnerability types, and uses **Arjun** to discover hidden parameters that normal crawlers miss.
...
| Feature | Details |
|---|---|
| Nuclei post-scan | Broad vuln scan (medium/high/critical) on live hosts after SQLMap |
| Arjun integration | Hidden HTTP parameter bruteforce on confirmed live hosts |
| GAU / Waybackurls | Historical URL harvest as extra recon source |
| Smart URL filter | Drops static assets (.css/.js/.svg) and cache-buster-only params |
| SQLMap power mode | Default level=3, risk=2, --forms, --tamper=space2comment, --threads=5 |
| --level / --risk | Configurable SQLMap depth from CLI |
| --tables / --dump | Enumerate tables or dump full contents from CLI |
| JSON export | Results saved to both CSV and JSON automatically |
| Scan timer | Total scan time shown at end of every run |
| New commands | `sqleasy logs`, `sqleasy report`, `sqleasy clear`, `sqleasy version` |
...
### Install everything in one command (Linux)
...
This diagram shows the complete v1.1.0 execution flow. Every box maps directly to a real module or function inside the codebase.
...
#### Step 3: Arjun - Hidden Parameter Discovery (v1.1.0)
...
#### Step 4: Historical URL Harvest - GAU / Waybackurls (v1.1.0)
...
| `--tamper` | space2comment | Basic WAF evasion via comment injection |
| `--timeout` | 10s | Per-request timeout |
| `--retries` | 2 | Auto-retry on connection failure |

**Post-scan Nuclei sweep (v1.1.0):**
...
flowchart TD
    ...
    J & K --> L{More files?}
    L -->|Yes| F
    L -->|No| M([CSV + JSON Export Complete])
```
...
flowchart TD
    ...
    MODE -->|none| DEP{All 4 core tools\ninstalled?}
    ...
    LOG1 & LOG2 --> TIMER[Print Scan Duration]
    TIMER --> DONE([Done])
```
...
| `sqleasy report` | Show full scan report summary (CSV + JSON) |
...
### Deep Extraction Mode (v1.1.0)
```bash
sqleasy start -d target.com --level 5 --risk 3 --dump
```

### Table Enumeration Only (v1.1.0)
```bash
...
*SQL Easy v1.1.0 - Released 2026-05-25 - [Apache 2.0 License](./LICENSE)*
