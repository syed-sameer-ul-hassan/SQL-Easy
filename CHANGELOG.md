...
### Added
- **`--resume` flag**: Skip recon and resume directly from `.targets.txt`.
- **`--target-list` flag**: Batch scan multiple domains from a plain-text file.
- **`--tamper` flag**: Auto-rotating tamper script pool (space2comment, between, randomcase, charencode, equaltolike).
- **`--config` flag**: Load default CLI flag values from `config.yaml`.
- **`--html` flag**: Generate dark-themed HTML report after every scan.
- **TOR detection**: Warns if TOR port 9050 is open but `--proxy` is not set.
- **Randomized URL ordering**: Shuffles URLs within priority tiers to defeat sequential pattern detection.
- **Gowitness integration**: Auto-screenshots confirmed vulnerable pages into `logs/screenshots/`.
- **HTML report generator**: `core/logging.py` generates styled `logs/report.html` from scan data.
- **Per-tool installer**: Both `install.sh` and `install.ps1` now ask `[y/n]` per tool with descriptions.
- **Termux (Android) support**: Full compatibility — auto-detects `$PREFIX/bin`, no `sudo`, arch-aware binaries.
- **Docker support**: Zero-dependency deployment via `Dockerfile`.
- **Unit tests**: `pytest` skeleton for all `core/` modules in `tests/`.
- **Config file example**: `config.yaml.example` documents all available flag defaults.

### Changed
- Updated all version strings from `v1.1.0` to `v1.2.0` across installers, wrapper, and docs.
- `install.sh` and `install.ps1` redesigned with per-tool `[y/n]` prompts instead of single bulk install.
- All binary download URLs are now architecture-aware (`amd64`, `arm64`, `arm`).
- Removed all `git` operations from `install.sh` and `install.ps1`.
- `core/scanner.py` now uses auto-rotating tamper pool instead of hardcoded `space2comment`.

### Fixed
- `SECURITY.md` version table corrected — `1.2.x` is current stable, not superseded.
- `README.md` diagrams updated to reflect v1.2.0 pipeline (config, resume, batch, html, gowitness, TOR).
